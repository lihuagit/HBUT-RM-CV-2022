...
bool PredictorAdaptiveEKF::predict(Detection_pack &data, RobotCmd &send, cv::Mat &im2show, bool is_show) {
    ...
    std::vector<bbox_t> detections;  // new_detection: vector 是经过过滤后所有可能考虑的装甲板
    ...
    /// 切换最重要装甲板，不管此刻击打的是谁
    if ((!selected && last_shoot && last_sbbox.tag_id != important_id) || (selected && armor.tag_id != important_id)) {
        for (auto &d: new_detections) {
    ...
    if (same_armor) {
        ...
        double d_b = -robot_speed_mps * robot_speed_mps -
