...
//	std::cout<<qt[0]<<qt[1]<<qt[2]<<qt[3]<<std::endl;
    ...
    std::vector<bbox_t> new_detections=detections;
    // // 过滤出敌方颜色的装甲板
    // for (auto &d: detections) {
    //      if (int(enemy_color) == d.color_id && d.confidence >=0.5f && d.tag_id != 0 && d.tag_id != 2 && d.tag_id != 6) new_detections.push_back(d);
    // }
    ...
    // for(int i=0;i<4;i++){
    //     std::cout<<"armor.pts "<<i<<":"<<armor.pts[i].x<<" "<<armor.pts[i].y<<std::endl;
    // }

    Eigen::Vector3d m_pc = pnp_get_pc(armor.pts, armor.tag_id);             // point camera: 目标在相机坐标系下的坐标
    Eigen::Vector3d pos_pc = pc_to_pu(m_pc);

    std::cout<<"pos_pc(0,0) "<<":"<<pos_pc(0,0)<<std::endl;
    std::cout<<"pos_pc(1,0) "<<":"<<pos_pc(1,0)<<std::endl;
    std::cout<<"pos_pc(2,0) "<<":"<<pos_pc(2,0)<<std::endl;

    std::cout << "m_pc0=" << m_pc(0,0) << std::endl;
    std::cout << "m_pc1=" << m_pc(1,0) << std::endl;
    std::cout << "m_pc2=" << m_pc(2,0) << std::endl;
    std::cout << "m_pc.norm()=" << m_pc.norm() << std::endl;
...
//    std::cout << "state: " << state << std::endl;
    ...
    double length_c = sqrt(m_pc(0, 0) * m_pc(0, 0) + m_pc(1, 0) * m_pc(1, 0));
    ...
    Eigen::Vector3d pc_pw{length_c * cos(mc_yaw), length_c * sin(mc_yaw), m_pc(2,0)};
...
//    std::cout << "t2:" << t_2 << std::endl;
	...
	re_project_point(im2show, pc_pw, R_IW, {0, 0, 255});
    for (int i = 0; i < 4; ++i)
...
bool PredictorKalman::predict_2(Detection_pack &data, RobotCmd &send, cv::Mat &im2show) {
//    std::cout << "In Kalman-predict" << std::endl;
